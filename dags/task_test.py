...
with DAG('spark_job_task_test', default_args=default_args, schedule_interval=None) as dag:
    ...
    local:///opt/spark/examples/jars/script.py
    """
	
	
    spark_submit_command1 = """
    /opt/spark/bin/spark-submit \
    --master=k8s://https://172.19.33.11:6443 \
    --deploy-mode cluster \
    --name script1 \
    ...
    run_spark_job1 = KubernetesPodOperator(
        task_id='run_spark_job1',
        namespace='default',
        image='kabileshe/newspark:3.5.1',  # Use the same image as in the spark-submit command
        cmds=["bash", "-c"],
        arguments=[spark_submit_command1],
        labels={"app": "spark"},
        get_logs=True,
    )
